...
디스크:: 고객 데이터는 프리미엄 스토리지 페이지 Blob에 있습니다. 각 노드는 다른 노드의 스토리지에 액세스할 수 있습니다. 의 경우 추가 스토리지도 필요합니다 link:reference_default_configs.html#boot-and-root-data-for-cloud-volumes-ontap["부팅, 루트 및 코어 데이터"].
스토리지 계정::
+
--
