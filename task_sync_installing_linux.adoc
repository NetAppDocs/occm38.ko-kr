...
+
URI입니다:: Cloud Sync 설치 파일의 URI가 지침 페이지에 표시됩니다. 이 내용은 화면의 지시에 따라 온-프렘 데이터 브로커를 배포할 때 로드됩니다. 이 URI는 링크가 동적으로 생성되고 한 번만 사용할 수 있으므로 여기서 반복되지 않습니다. <<데이터 브로커 설치,다음 단계에 따라 Cloud Sync에서 URI를 가져옵니다>>.
