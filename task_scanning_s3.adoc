...
==== image:number1.png["1번"] 클라우드 환경에서 S3 요구사항을 설정합니다

[role="quick-margin-para"]
IAM 역할 준비 및 Cloud Compliance에서 S3로 연결 설정 등 클라우드 환경이 클라우드 규정 준수 요구 사항을 충족할 수 있는지 확인합니다. <<S3 사전 요구 사항 검토,전체 목록을 참조하십시오>>.
...
==== image:number3.png["3번"] S3 작업 환경에서 규정 준수를 활성화합니다
...
==== image:number4.png["4번"] 스캔할 버킷을 선택합니다
...
클라우드 규정 준수도 가능합니다 <<추가 AWS 계정에서 버킷 스캔,서로 다른 AWS 계정에 있는 S3 버킷을 스캔합니다>>.
